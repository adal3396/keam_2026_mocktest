...
import { ArrowLeft, Plus, Trash2, Save, ImagePlus, X, Sigma } from 'lucide-react';
          ...
          <Card key={idx} className="relative">
            <CardContent className="pt-6 space-y-4">
              ...
              <div className="space-y-2">
                ...
                {(q.questionText.includes('$') || q.questionText.includes('$$')) && (
                  <div className="mt-2 p-3 bg-muted/30 rounded-lg border border-dashed">
                    <div className="flex items-center gap-2 mb-2 text-[10px] font-bold uppercase tracking-wider text-muted-foreground">
                      <Sigma className="w-3 h-3" /> Math Preview
                    </div>
                    <MathRenderer text={q.questionText} className="text-sm" />
                  </div>
                )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {(['A', 'B', 'C', 'D', 'E'] as const).map(opt => (
                  <div key={opt} className="space-y-1">
                    ...
                    />
                    {((opt === 'A' ? q.optionA : opt === 'B' ? q.optionB : opt === 'C' ? q.optionC : opt === 'D' ? q.optionD : q.optionE)?.includes('$')) && (
                      <div className="mt-1 p-2 bg-muted/20 rounded border border-dashed text-xs">
                         <MathRenderer text={opt === 'A' ? q.optionA : opt === 'B' ? q.optionB : opt === 'C' ? q.optionC : opt === 'D' ? q.optionD : q.optionE} />
                      </div>
                    )}
                  </div>
                ))}
